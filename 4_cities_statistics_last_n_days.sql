WITH dates AS (
    SELECT 
		DATE(MAX(reservation_created_date)) AS last_week_end,
		DATE(MAX(reservation_created_date) - INTERVAL '6 days') AS last_week_start
    FROM reservations
),

reservations_by_city AS (
    SELECT 
        m.city,
        r.country,
    ...
    WHERE 
        DATE(r.reservation_created_date) >= (SELECT last_week_start FROM dates)
        AND DATE(r.reservation_created_date) <= (SELECT last_week_end FROM dates)
    GROUP BY 
        m.city, r.country
),
...
total_metrics AS (
    ...
    FROM 
        reservations
    WHERE 
        DATE(reservation_created_date) >= (SELECT last_week_start FROM dates)
        AND DATE(reservation_created_date) <= (SELECT last_week_end FROM dates)
)
